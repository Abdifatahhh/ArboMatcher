    ...
    <div className="p-6">
      ...
      {applications.length === 0 ? (
        <div className="bg-white p-12 rounded-lg shadow-lg text-center">
          ...
          <h3 className="text-xl font-semibold text-slate-700 mb-2">Geen kandidaten</h3>
          <p className="text-slate-600">Er hebben nog geen professionals gereageerd op uw opdrachten</p>
        </div>
      ) : (
          ...
          {applications.map((application) => (
            <div key={application.id} className="bg-white p-6 rounded-lg shadow-lg">
              ...
              {application.professionals.bio && (
                <p className="text-slate-700 mb-4">{application.professionals.bio.substring(0, 200)}...</p>
              )}

              {application.message && (
                <div className="mb-4 p-4 bg-slate-50 rounded">
                  <p className="text-sm text-slate-600 font-semibold mb-2">Motivatie:</p>
                  ...
                  {application.status === 'PENDING' && (
                    <>
                      <button
                        ...
                        className="px-4 py-2 bg-gradient-to-r from-emerald-500 to-green-400 text-white rounded-lg hover:from-emerald-600 hover:to-green-500 transition shadow-lg shadow-emerald-500/20"
                      >
                        Shortlist
                      </button>
                      <button
