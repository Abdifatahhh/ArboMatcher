    ...
    <div className="p-6 sm:p-8">
      ...
      <div className="space-y-3">
        {DUMMY_ARTISTS.map((a, i) => (
          <div
            key={i}
            className="flex items-center gap-3 py-3 px-4 bg-gray-50 rounded-xl border border-gray-100 hover:bg-emerald-50/50 hover:border-emerald-100 transition-colors"
          >
            <div className="w-10 h-10 rounded-full bg-[#4FA151]/10 flex items-center justify-center flex-shrink-0">
              <span className="text-[#4FA151] font-bold text-sm">{a.name.charAt(0)}</span>
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-medium text-[#0F172A] text-sm">{a.name}</p>
              <p className="text-xs text-gray-500">{a.specialism} · {a.region}</p>
            </div>
            {a.verified && (
              <span className="flex items-center gap-1 text-xs font-medium text-[#4FA151]">
    ...
    <div className="p-6 sm:p-8">
      ...
      <div className="mb-5">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-medium text-gray-600">Kwaliteitscore</span>
          <span className="text-sm font-semibold text-[#4FA151]">80%</span>
        </div>
        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
          <div
            className="h-full bg-[#4FA151] rounded-full transition-all duration-500"
            style={{ width: '80%' }}
          />
        </div>
      </div>
      <div className="space-y-3">
        {[
          ...
          <div key={i} className="flex items-center gap-3 py-2">
            {item.done ? (
              <div className="w-6 h-6 rounded-full bg-[#4FA151] flex items-center justify-center flex-shrink-0">
            ...
            <span className={item.done ? 'text-gray-500 line-through text-sm' : 'text-[#0F172A] font-medium text-sm'}>
              {item.label}
            </span>
          </div>
        ))}
...
function PreviewStep3() {
  return (
    <div className="p-6 sm:p-8">
      <div className="flex items-center gap-2 mb-6">
        <UserCheck className="w-5 h-5 text-[#4FA151]" />
        ...
        {[
          ...
          <div
            ...
            className={`flex items-center gap-3 py-3 px-4 rounded-xl border transition-colors ${
              c.new ? 'bg-[#4FA151]/5 border-[#4FA151]/20' : 'bg-gray-50/50 border-gray-100'
            }`}
          >
            <div className="w-2 h-2 rounded-full bg-[#4FA151] flex-shrink-0" style={{ opacity: c.new ? 1 : 0 }} />
            <div className="flex-1 min-w-0">
              <p className="font-medium text-[#0F172A] text-sm">{c.name}</p>
              <p className="text-xs text-gray-500">heeft gereageerd · {c.time}</p>
            </div>
    ...
    <div className="p-6 sm:p-8">
      ...
      <div className="space-y-3">
        {['Op shortlist', 'Uitgenodigd voor gesprek', 'Geselecteerd'].map((label, i) => (
          <div
            key={i}
            className="flex items-center gap-3 py-3 px-4 rounded-xl border border-gray-100 bg-gray-50/50"
          >
            <span className="flex-shrink-0 w-8 h-8 rounded-lg bg-[#4FA151]/10 flex items-center justify-center text-[#4FA151] font-bold text-sm">
              {i + 1}
            </span>
            <span className="text-[#0F172A] font-medium text-sm">{label}</span>
            {i === 2 && (
              <CheckCircle className="w-5 h-5 text-[#4FA151] ml-auto flex-shrink-0" />
            )}
          </div>
        ))}
      </div>
...
function PreviewStep5() {
  return (
    <div className="p-6 sm:p-8">
      <div className="flex items-center gap-2 mb-6">
        <Handshake className="w-5 h-5 text-[#4FA151]" />
        <span className="font-semibold text-[#0F172A]">Samen aan het werk</span>
      </div>
      <div className="rounded-xl border border-gray-200 bg-[#4FA151]/5 p-4 space-y-3">
