...
interface OrganisatieDashboardLayoutProps {
...
export function OrganisatieDashboardLayout({ children }: OrganisatieDashboardLayoutProps) {
        ...
        <div key={gi}>
          ...
          <div className="space-y-0.5">
            {group.items.map((item) => {
              ...
              const isActive = location.pathname === item.path || (item.path !== '/organisatie/dashboard' && location.pathname.startsWith(item.path));
              return (
                <Link
                  key={item.path}
                  to={item.path}
      ...
      <header className="flex items-center w-full bg-white border-b border-slate-200 shrink-0">
        ...
        <div className="flex-1 flex items-center justify-between px-3 py-2.5 md:px-4 md:py-3 lg:px-6 min-w-0">
          ...
          <div className="flex items-center gap-1 ml-auto">
            <Link to="/organisatie/inbox" className="p-2 rounded-lg hover:bg-slate-100 text-slate-700 lg:hidden">
            ...
            <Link to="/organisatie/profiel" className="p-1.5 rounded-full hover:bg-slate-100 text-slate-700 flex items-center justify-center w-9 h-9 bg-slate-50 border border-slate-200 lg:hidden">
              {profile?.avatar_url ? (
      ...
      <div className="flex flex-1 min-w-0">
        <aside className="hidden lg:flex lg:flex-col lg:w-64 bg-white border-r border-slate-200">
          ...
          <div className="border-t border-slate-100 p-3 lg:p-4 space-y-1">
            <Link to="/organisatie/profiel" className={`flex items-center space-x-3 px-3 py-2 lg:px-4 lg:py-2.5 rounded-lg text-sm ${location.pathname === '/organisatie/profiel' ? navActive : navInactive}`}>
            ...
            <div className="fixed inset-0 z-50 lg:hidden">
              ...
              <div className="absolute bottom-0 left-0 right-0 max-h-[75vh] flex flex-col bg-white rounded-t-2xl overflow-hidden animate-in slide-in-from-bottom duration-200 border-t border-slate-200 shadow-lg">
                ...
                <div className="p-3 border-t border-slate-100 space-y-1">
                  <Link to="/organisatie/profiel" onClick={() => setBottomSheetOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-700 hover:bg-slate-50 text-sm">
